...
public class FightManager : MonoBehaviour
{
    ...
    IEnumerator Attack(Character teamACharacter, Character teamBCharacter)
    {
        ...
        if (teamABattlePoints > teamBBattlePoints)
        {
            ...
            outcome = (float)teamABattlePoints / (teamABattlePoints + teamBBattlePoints);// this really needs to be a fraction of the win vs the loser, but if it's a draw 0 is okay.
            Debug.Log("Team A | " + outcome);
            winner.AddXP(5);
        ...
        else if (teamABattlePoints < teamBBattlePoints)
        {
            ...
            outcome =  (float)teamBBattlePoints / (teamABattlePoints + teamBBattlePoints);// this really needs to be a fraction of the win vs the loser, but if it's a draw 0 is okay.
            Debug.Log("Team B | " + outcome);
            winner.AddXP(5);
            BattleLog.Log("Team B Wins!", winBCol);
            
        }
        else //(teamABattlePoints == teamBBattlePoints)
        {
            winner = teamACharacter;
            defeated = teamBCharacter;
        ...
        Debug.LogWarning("Attack called, BattleLog called");
